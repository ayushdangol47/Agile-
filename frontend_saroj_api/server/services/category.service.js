...
async function create(req,result){
    let category = {Id: uuidv1(),Name : req.body.Name,Image:req.body.Image}
...
async function getById(id,result) {
    sql.query("select Id,Name,Image from category where id = ? ", id, function (err, data) {             
        if(err) {
            result(err, null);
        }
        else{
...
async function update(req,result) {
    sql.query("update category set Name=?,Image=? where Id = ? ", [req.body.Name,req.body.Image,req.body.Id], function (err, data) {             
        if(err) {
            result(err, null);
        }
        else{
...
async function getPaginatedList(req,result) {
    ...
    sql.query("select SQL_CALC_FOUND_ROWS Id,  Id, Name , Image from category LIMIT ?, ?;SELECT FOUND_ROWS() as total;",[skip,pagesize],  function (err, response) {             
